...
一个面向「心意表达 + 互动抽奖」场景的 Web 应用。你可以快速创建活动、配置玩法与奖品、设置分享权限，并通过链接邀请参与者完成抽奖互动。
<img width="3820" height="1946" alt="image" src="https://github.com/user-attachments/assets/51a10d3c-d028-43fa-a43a-4c4ae352c80e" />


## 核心能力

- 多玩法抽奖：大转盘、老虎机、翻卡牌、盲盒、刮刮卡
- 活动编辑器：文案、奖品、主题、封面、实时预览、自动保存
- 分享权限：公开访问 / 口令访问 / 关闭访问
- 分享体验：链接复制、二维码生成/复制/下载、系统分享
- 数据统计：访问次数、抽奖次数、中奖记录（含抽奖时快照）
...
- 头像与用户上传图像增加懒加载与异步解码，默认头像内置资源兜底
- 登录/注册页去除多余嵌套结构，减少无效层级
- 保持关键页面样式统一，降低渲染抖动与重排风险
- 刮刮卡触摸链路优化（移动端跟手性 + 擦除检测节流）

## 最近更新

- 统一交互视觉反馈：hover / active / focus 样式一致化
- 修复点击后边框高亮观感，改为更干净的按压与 ring 反馈
- 新增 `TA 的照片` 默认头像，并在编辑页/分享页/日志页全链路兜底
